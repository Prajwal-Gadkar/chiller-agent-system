"""
Forecast Agent v2 — Thermal Load (Flow x Delta_T) -> Power prediction per chiller.
Extends forecast_agent.py by adding CHW Return/Leave Temperature and Delta_T
alongside Flow, since chiller power draw physically depends on cooling thermal load.

Per CLAUDE.md:
- NEVER pool chillers — each chiller gets its own independent model.
- Uses controllable features (Flow, Delta_T, Thermal_Load) to predict outcome target (Power/KW).
...
def identify_sensor_columns(df_chiller, m_id, df_types):
    """
    Identify Flow, Power, Return Temp, and Leave Temp columns for a machineId.
    """
    ...
    # Return Temp candidates (inlet/return)
    ret_candidates = [c for c in pop_cols if any(k in c.lower() for k in ["chw return", "evaporator_inlet", "inlet_temperature"])]
    ret_temp_col = ret_candidates[0] if ret_candidates else None

    # Leave Temp candidates (outlet/leave)
    leave_candidates = [c for c in pop_cols if any(k in c.lower() for k in ["chw leave", "evaporator_outlet", "outlet_temperature"])]
    leave_temp_col = leave_candidates[0] if leave_candidates else None

    return flow_col, power_col, ret_temp_col, leave_temp_col


def train_and_evaluate_chiller_v2(chiller_df, m_id, chiller_type, flow_col, power_col, ret_temp_col, leave_temp_col):
    """
    Train and evaluate a multi-feature physics model (Flow, Delta_T, Thermal_Load) -> Power for a chiller.
    """
    if not flow_col or not power_col or not ret_temp_col or not leave_temp_col:
        return {
            "machineId": m_id,
            "chiller_type": chiller_type,
            "flow_column": str(flow_col),
            "power_column": str(power_col),
            "ret_temp_column": str(ret_temp_col),
            "leave_temp_column": str(leave_temp_col),
            "test_R2": np.nan,
            "cv_R2_mean": np.nan,
            "cv_R2_std": np.nan,
            "RMSE": np.nan,
    ...
    # Run Data Validation Agent gate
    flagged_df, _ = validate(chiller_df)

    # Required columns
    cols_to_check = [flow_col, power_col, ret_temp_col, leave_temp_col]
    ...
    clean_df = flagged_df[valid_mask].sort_values("timestamp").copy().reset_index(drop=True)
    ...
    if len(clean_df) < 50:
        return {
            ...
            "ret_temp_column": ret_temp_col,
            "leave_temp_column": leave_temp_col,
            "test_R2": np.nan,
            "cv_R2_mean": np.nan,
            "cv_R2_std": np.nan,
            "RMSE": np.nan,
    ...
    # Compute Delta_T and Thermal_Load = Flow * Delta_T
    clean_df["Delta_T"] = clean_df[ret_temp_col] - clean_df[leave_temp_col]
    clean_df["Thermal_Load"] = clean_df[flow_col] * clean_df["Delta_T"]

    # Feature matrix X (Flow, Delta_T, Thermal_Load) and Target y (Power)
    ...
    # MAPE (clip tiny y_test denominators to prevent division by near-zero spikes)
...
def main():
    parser = argparse.ArgumentParser(description="Forecast Agent v2: (Flow, Delta_T, Thermal_Load) -> Power prediction per chiller.")
    parser.add_argument(
        "--wide-input",
    ...
    parser.add_argument(
        "--output",
        default=os.path.join(PROJECT_ROOT, "data", "forecast_agent_v2_results.csv"),
        help="Path to save forecast_agent_v2_results.csv"
    )
    ...
    print(f"Running Forecast Agent v2 on {len(clean_ids)} clean group chillers...")
    ...
    for m_id in clean_ids:
        ...
        flow_col, power_col, ret_temp, leave_temp = identify_sensor_columns(chiller_df, m_id, df_types)
        res = train_and_evaluate_chiller_v2(chiller_df, m_id, c_type, flow_col, power_col, ret_temp, leave_temp)
        results.append(res)
    ...
    print("\n" + "="*95)
    print("FORECAST AGENT V2 (THERMAL LOAD: FLOW x DELTA_T -> POWER) MODEL RESULTS")
    print("="*95)
